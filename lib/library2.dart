...
import 'package:psych_app/library2_topic_detail.dart';
...
class DearMeLibrary extends StatefulWidget {
  const DearMeLibrary({Key? key}) : super(key: key);

  @override
  DearMeLibraryState createState() => DearMeLibraryState();
}

class DearMeLibraryState extends State<DearMeLibrary> with AutomaticKeepAliveClientMixin, TickerProviderStateMixin<DearMeLibrary> {
...
// 4 selectedEmotion
  void getLibraryData() async {
    ...
    if(await checkInternetConnectivity()){
      ...
      var result = await get(Uri.parse(APIUrls.getUrl("${APIUrls.library}${APIUrls.pass}/$role/$offset/1/All", queryParams)), headers: {"Accept": "application/json"});
      // print(result.body);
      ...
      if(jsonObject['status'] == 200){
        ...
        if(requests.isNotEmpty){
          // convert to list
          list1 = requests.map<Library>((json) => Library.fromJson(json)).toList();
        ...
        Builder(builder: (context) => Column(
          ...
          children: <Widget>[
                   ...
                   Container(
                    ...
                    child:  
                      Row(
                        ...
                        Text('Library', style: GoogleFonts.dmSerifText(textStyle: Theme.of(context).textTheme.displaySmall, fontWeight: FontWeight.bold)), 
                      ],
                    ),
           ...
           ////////
            ...
            //               DropdownMenuItem(
            //                   value: e,
            //                   child: Text(e, style: GoogleFonts.dmSans(textStyle: Theme.of(context).textTheme.bodyLarge)),
            //                 ),
            //             ).toList(),
                        ...
                        child: 
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Container(
                                alignment: Alignment.center,
                                child: Text('No data available!', style: GoogleFonts.dmSans(textStyle: Theme.of(context).textTheme.bodyText2)), 
                            ),
                            sizedBox(8),
...
Widget myRequestCard(int position, BuildContext context){
  return InkWell(
    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => LibraryDetail(feeling: list[position]))),
        ...
        child: 
          Column(
            ...
            children: <Widget>[
                  ...
                  Row(
                    ...
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: <Widget>[
                        ...
                        Image.network(list[position].media!, height: 120,) : sizedBox(0),
                        // sizedBox(16),
                        Expanded(child: 
                        Text(list[position].title!, style: GoogleFonts.dmSans(fontSize: 20, fontWeight: FontWeight.bold)), 
                        // Text(list[position].feeling!, style: GoogleFonts.dmSerifText(textStyle: Theme.of(context).textTheme.displaySmall)), 
                        )
                  ...
                  // Text('Last assessed on: '  + DateFormat('MMM dd, yyyy · hh:mm aa', 'en_US').format(getDate(list[position].assessmentOn!)), style: GoogleFonts.dmSans(textStyle: Theme.of(context).textTheme.bodySmall))
                  // : sizedBox(0), 
